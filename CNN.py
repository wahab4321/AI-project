...
def ModelCNN(InpSymbol="U30USD.HKT"):
    ...
    def get_model_result(trial, eval=False):
        ...
        model.add(tf.keras.layers.Dense(X_train.shape[1], input_shape=(X_train.shape[1], 1)))
        model.add(tf.keras.layers.Conv1D(trial.suggest_int('conv', 3, 32), 4, input_shape=(X_train.shape[1], 1)))
        ...
        model.fit(X_train.astype(np.float32), y_train.astype(np.float32), validation_data=(X_val.astype(np.float32), y_val.astype(np.float32)), epochs=30, batch_size=32, callbacks=[model_checkpoint_callback, early_stopping_callback])
